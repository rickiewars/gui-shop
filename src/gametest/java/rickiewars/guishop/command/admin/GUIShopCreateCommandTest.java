...
public class GUIShopCreateCommandTest extends CommandTestBase {
    ...
    public void createShopAddsShopWithSlugifiedId(GameTestHelper context) {
        ...
        assertTrue(context, capture.anyMessageContains("successfully created"), "expected a success message");
        assertValueEqual(context, GUIShop.shops.size(), 1, "shop count after create");
        assertValueEqual(context, GUIShop.shops.get(0).getId(), "cool_shop", "slugified shop id");
        assertValueEqual(context, GUIShop.shops.get(0).getDisplayName(), "Cool Shop!", "shop display name");
        context.succeed();
    }
    ...
    public void createShopWithCollidingSlugGetsUniqueSuffix(GameTestHelper context) {
        ...
        assertValueEqual(context, GUIShop.shops.size(), 2, "shop count after two colliding creates");
        assertValueEqual(context, GUIShop.shops.get(0).getId(), "cool_shop", "first shop id");
        assertValueEqual(context, GUIShop.shops.get(1).getId(), "cool_shop_2", "second shop id");
        context.succeed();
    }
