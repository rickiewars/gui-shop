...
public class GUIShopBalanceSendCommandTest extends EconomyCommandTest {
    ...
    public void sendMovesBalanceBetweenPlayers(GameTestHelper context) {
        ...
        context.assertTrue(capture.anyMessageContains("Successfully sent"), "expected a success message");
        context.assertValueEqual(senderAccount.balance(), 70L, "sender balance after send");
        context.assertValueEqual(receiverAccount.balance(), 30L, "receiver balance after send");
        context.succeed();
    }
    ...
    public void sendRejectsTargetingSelf(GameTestHelper context) {
        ...
        context.assertTrue(capture.anyMessageContains("cannot target yourself"), "expected a target-self message");
        context.assertValueEqual(senderAccount.balance(), 100L, "sender balance after a self-send attempt");
        context.succeed();
    }
    ...
    public void sendFailsWithInsufficientFunds(GameTestHelper context) {
        ...
        context.assertTrue(capture.anyMessageContains("Transaction failed"), "expected a transaction-failed message");
        context.assertValueEqual(senderAccount.balance(), 0L, "sender balance after insufficient funds");
        context.assertValueEqual(receiverAccount.balance(), 0L, "receiver balance after insufficient funds");
        context.succeed();
    }
